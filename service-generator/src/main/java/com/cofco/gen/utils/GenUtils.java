...
import com.cofco.gen.entity.ColumnEntity;
...
public class GenUtils {
	...
	public static void generatorCode(Map<String, String> table,
			List<Map<String, String>> columns, ZipOutputStream zip,int genType , String path){
		//配置信息
...
//			columnEntity.setExtra(column.get("extra"));
			...
			//列的数据类型，转换成Java类型
			columnEntity.setDataType(columnEntity.getDataType().toLowerCase());
		...
		for(String template : templates){
			...
			if(genType == Constant.genType.local.getValue()){
                local(template,tableEntity,tpl,context,path);
			}
			//web页面生成
	 ...
	 * @param context
	 */
	public static void local(String template,TableEntity tableEntity,Template tpl,VelocityContext context,String path){
		try {
			...
			if(!file .exists()  && !file .isDirectory()){
				String filePaths = filePath.substring(filePath.lastIndexOf("\\")+1);
				File fileDir = new File(filePath.substring(0,(filePath.length() - filePaths.length())));
				fileDir.mkdirs();
				file.createNewFile();
			}
			//mysql创建文件 判断
